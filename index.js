...
async function run() {
  try {
    ...
    //  Get Latest 6 Data (sorted by date ascending)
    app.get('/latest-data', async (req, res) => {
   try {
     const latestIssues = await issuesCollection
       .find({})
       .sort({ date: -1 }) // descending order (latest first)
       .limit(6)
       .toArray();
    ...
    // DELETE: Delete issue by ID from AllIssue collection
    ...
    // PUT: Update issue by ID in AllIssue collection
